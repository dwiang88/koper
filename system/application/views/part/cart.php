			...
			<div style="clear:both"></div>
				...
				<div id="content-right">
                                    ...
                                    <form action="<?php echo site_url('cart/buy');?>" method="POST">
                                        <table>
                                            <tr><td>No</td><td>Nama Barang</td><td>ID Barang</td><td>Jumlah Barang</td><td>Harga per Satuan</td><td>Total Harga</td></tr>
                                        <?php
                                            $count=1;
                                            $total=0;
                                            foreach($this->cart->contents() as $row){
                                                $temp =$row['price']*$row['qty'];
                                                echo "<tr><td>$count</td><td>$row[name]</td><td>$row[id]</td><td>$row[qty]</td><td>$row[price]</td><td>$temp</td></tr>";
                                                $count++;
                                                $total+=$temp;
                                            }
                                            echo "<tr><td colspan='5'>Total Harga Pembelian</td><td>$total</td>";
                                        ?>
